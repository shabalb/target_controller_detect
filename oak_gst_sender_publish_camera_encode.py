#!/usr/bin/env python3
"""
OAK Camera → H.264 RTP/UDP sender.

Captures frames from OAK (DepthAI v3 API), encodes H.264 on the OAK
VideoEncoder node, then pushes the encoded byte stream into GStreamer
for RTP/UDP packetization.

Dependencies:
    pip install depthai>=3.0
    GStreamer 1.x with h264parse/rtph264pay
...
def build_gst_pipeline(host: str, port: int, width: int, height: int, fps: int) -> str:
    ...
    appsrc receives an already encoded H.264 Annex-B byte stream from OAK.
    """
    return (
...
def configure_encoder_bitrate(encoder, bitrate: int) -> None:
    if hasattr(encoder, "setBitrate"):
        encoder.setBitrate(int(bitrate))
    elif hasattr(encoder, "setBitrateKbps"):
        encoder.setBitrateKbps(int(bitrate // 1000))


def create_h264_encoder(pipeline, video_out, fps: int, bitrate: int):
    profile = dai.VideoEncoderProperties.Profile.H264_MAIN
    encoder = pipeline.create(dai.node.VideoEncoder)
    if hasattr(encoder, "build"):
        encoder = encoder.build(video_out, frameRate=float(fps), profile=profile)
    else:
        encoder.setDefaultProfilePreset(int(fps), profile)
        video_out.link(encoder.input)
    configure_encoder_bitrate(encoder, bitrate)
    return encoder


def get_encoder_output(encoder):
...
def build_depthai_pipeline(
    ...
    # NV12 is the encoder-friendly ISP output. It is encoded on the OAK,
    # so Radxa receives compressed H.264 instead of 1080p raw frames.
    video_out = cam.requestOutput(
        size=(video_width, video_height),
        type=dai.ImgFrame.Type.NV12,
        fps=float(video_fps),
    )
    encoder = create_h264_encoder(pipeline, video_out, video_fps, video_bitrate)
    video_queue = get_encoder_output(encoder).createOutputQueue(maxSize=4, blocking=False)

    rgb_queue = None
    if enable_shm:
        rgb_out = cam.requestOutput(
            size=(shm_video_width, shm_video_height),
            type=dai.ImgFrame.Type.BGR888p,
            fps=float(shm_video_fps),
        )
    ...
    return pipeline, video_queue, rgb_queue, depth_queue
...
class OakGstSender:
    ...
    def start(self):
        ...
        if self._rgb_queue is not None:
            self._rgb_thread = threading.Thread(
                target=self._rgb_loop,
                name="oak-shm-rgb",
                daemon=True,
            )
    ...
    def stop(self):
        ...
        for worker in (self._video_thread, self._rgb_thread, self._depth_thread):
    ...
    def _video_loop(self):
        while self.running and self._dai_pipeline and self._dai_pipeline.isRunning():
            ...
            # getData() returns H.264 access units encoded on the OAK.
            frame_data = frame_msg.getData()
            if not self._reported_video_format:
                print(
                    f"[DepthAI] GST H.264 frame: "
                    f"{CAM_WIDTH}x{CAM_HEIGHT}@{CAM_FPS}, {len(frame_data)} bytes"
                )
    ...
    def _rgb_loop(self):
        while self.running and self._dai_pipeline and self._dai_pipeline.isRunning():
            frame_msg = self._rgb_queue.tryGet()
            if frame_msg is None:
                time.sleep(0.001)
                continue
            self._maybe_write_rgb_shm(frame_msg)

    def _maybe_write_rgb_shm(self, frame_msg):
        ...
        try:
            ...
            self._last_shm_rgb_time = time.monotonic()
