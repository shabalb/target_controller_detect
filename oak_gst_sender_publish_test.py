...
def nv12_to_bgr_bytes(data: bytes, width: int, height: int) -> bytes:
    ...
    if cv2 is not None:
        nv12 = np.frombuffer(data[:expected_size], dtype=np.uint8).reshape(
            (height * 3 // 2, width)
        )
        return cv2.cvtColor(nv12, cv2.COLOR_YUV2BGR_NV12).tobytes()
...
class OakGstSender:
    ...
    def __init__(
        ...
        self._reported_image_format = False
    ...
    def start(self):
        ...
        self._image_thread = None
        self._depth_thread = None
        if self.enable_gst and self._video_queue:
        ...
        if self.enable_image and self._image_queue:
        ...
        if self.enable_depth and self._depth_queue:
    ...
    def _image_loop(self):
        while self.running and self._dai_pipeline and self._dai_pipeline.isRunning():
            ...
            if not self._reported_image_format:
                width, height = get_frame_size(
                    image_msg, self.image_width, self.image_height
                )
                print(
                    f"[DepthAI] ROS image frame: "
                    f"{width}x{height}, source={self._image_source_format}"
                )
                self._reported_image_format = True
