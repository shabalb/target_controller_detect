...
private:
  ...
  target_controller_detect::msg::Detection2D infer(
    ...
    RCLCPP_INFO(get_logger(), "infer net forward in");
    try {
      ...
      RCLCPP_ERROR(
        get_logger(), "OpenCV DNN forward failed (%d/3): %s", forward_error_count_, exc.what());
      if (forward_error_count_ >= 3) {
        model_ready_ = false;
        RCLCPP_ERROR(
          get_logger(),
          "Disabling NN model after repeated forward failures. "
          "This usually means the ONNX model is not compatible with the installed OpenCV DNN.");
      }
      return detection;
    }
    RCLCPP_INFO(get_logger(), "infer net forward out");
    if (outputs.empty()) {
      return detection;
    }
