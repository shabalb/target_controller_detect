...
The high-rate NV12 stream is pushed to GStreamer for hardware H.264 encoding.
ROS publishes a lower-rate BGR image for detectors and an organized PointCloud2
from aligned stereo depth for target_fusion_node.
"""
...
DEFAULT_CLOUD_TOPIC = "/oak/stereo/depth/points"
...
def build_gst_pipeline(host: str, port: int, width: int, height: int, fps: int) -> str:
    return (
        ...
        f"  max-buffers=2 leaky-type=downstream format=time "
        f'  caps=video/x-raw,format=NV12,width={width},height={height},framerate={fps}/1 '
        f"! mpph264enc "
        f"    bps={4_000_000} bps-max={6_000_000} rc-mode=cbr "
        f"    gop={fps} header-mode=1 profile=high "
...
class OakGstSender:
    ...
    def start(self):
        ...
        self._ros_node = rclpy.create_node("oak_gst_sender_pub")
        qos = make_sensor_qos()
        ...
        pipeline_str = build_gst_pipeline(self.host, self.port, CAM_WIDTH, CAM_HEIGHT, CAM_FPS)
